...
<aside id="app-sidebar" class="fixed inset-y-0 left-0 z-40 flex flex-col w-64 -translate-x-full bg-brandNavy transition-all duration-300 lg:translate-x-0 lg:static lg:z-auto">
    <div class="h-16 flex items-center justify-between px-6 border-b border-white/10">
        <div class="flex items-center min-w-0">
            ...
            <h1 class="sidebar-label font-heading text-base font-semibold tracking-tight text-white truncate">AITSA Staff</h1>
        </div>
    ...
    <nav class="flex-1 overflow-y-auto py-5 px-3 space-y-0.5">
        <p class="sidebar-label px-3 text-[11px] font-semibold text-white/40 mb-3">Management</p>
        @foreach ($cashierNavLinks as $link)
            <a href="{{ route($link['route']) }}" title="{{ $link['label'] }}"
               class="flex items-center space-x-3 px-3 py-2.5 border-l-2 text-sm transition-colors {{ Route::is($link['route']) ? 'border-brandGold bg-white/5 text-white font-semibold' : 'border-transparent text-white/60 hover:text-white font-medium' }}">
