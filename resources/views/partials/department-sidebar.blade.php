...
<aside id="app-sidebar" class="fixed inset-y-0 left-0 z-40 flex flex-col w-64 -translate-x-full bg-white dark:bg-panelDark border-r border-brandNavy/10 dark:border-slate-800 transition-all duration-300 lg:translate-x-0 lg:static lg:z-auto">
    <div class="h-16 flex items-center justify-between px-6 border-b border-brandNavy/10 dark:border-slate-800">
        <div class="flex items-center min-w-0">
            ...
            <h1 class="sidebar-label text-base font-black tracking-tight text-brandNavy dark:text-white truncate">AITSA</h1>
        </div>
    ...
    <nav class="flex-1 overflow-y-auto py-5 px-3 space-y-0.5">
        <p class="sidebar-label px-3 text-[10px] font-bold text-brandNavy/40 dark:text-slate-500 uppercase tracking-widest mb-3">{{ Auth::user()->department->name ?? 'Department' }}</p>
        <a href="{{ route('department.dashboard') }}" title="Clearance Queue" class="flex items-center space-x-3 px-3 py-2.5 border-l-2 border-brandGreen text-brandGreen dark:text-emerald-400 font-bold text-sm">
