    ...
    <div style="max-width:480px; margin:0 auto; padding:32px 24px;">
        ...
        <div style="background:#ffffff; border-radius:0 0 12px 12px; padding:28px 24px; border:1px solid #e5e7eb; border-top:none;">
            ...
            @if($submission->status === 'pending')
                <p style="font-size:14px; line-height:1.6;">This is a reminder that your <strong>{{ $submission->typeLabel() }}</strong> is still pending review by the Registrar's Office.</p>
            @else
                <p style="font-size:14px; line-height:1.6;">This is a reminder that your <strong>{{ $submission->typeLabel() }}</strong> was rejected and needs your attention.</p>
                @if($submission->remarks)
                    <div style="background:#fef2f2; border:1px solid #fecaca; border-radius:8px; padding:14px 16px; margin:16px 0;">
                        <p style="margin:0; font-size:13px; color:#991b1b;"><strong>Reason:</strong> {{ $submission->remarks }}</p>
                    </div>
                @endif
            @endif
            <p style="font-size:13px; line-height:1.6; color:#4b5563;">Please log in to the AitsaGate Portal to review your document status.</p>
