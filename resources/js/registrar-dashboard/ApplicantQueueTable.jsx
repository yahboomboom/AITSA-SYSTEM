...
export default function ApplicantQueueTable({ applicants, csrfToken }) {
    ...
    // Shown in full by default — every pending applicant is already loaded
    // (see registrar.dashboard), so there's no reason to hide them behind a
    // search first. The box just narrows the list down from there.
    ...
    return (
        <>
            <div className="px-6 py-4 border-b border-brandNavy/10 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center sm:justify-end gap-3">
                <input
                    type="text"
                    value={search}
                ...
                <table className="ui-table">
                    ...
                    <tbody>
                        {filteredApplicants.length === 0 ? (
                            <tr><td colSpan={9} className="border-brandNavy/8 dark:border-slate-800 p-8 text-center text-brandNavy/40 dark:text-slate-500">{query ? 'No matching applicants.' : 'No pending applications.'}</td></tr>
                                ...
                                <td className="border-brandNavy/8 dark:border-slate-800">
                                    <div className="flex flex-wrap items-center justify-center gap-1.5">
                                        ...
                                        {/* For applicants who never opted into an online/counter reservation
                                            ...
                                            Styled as a solid, distinctly-colored button since this is an
                                            irreversible action that creates real login credentials and emails
