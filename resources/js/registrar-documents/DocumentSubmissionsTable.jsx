...
export default function DocumentSubmissionsTable({ searchUrl, pendingCount, rejectedCount, csrfToken }) {
    // A student ID arriving via ?search=... (from the clearance queue's "View"
    // button, on the Dashboard page) takes priority; with no deep link, this
    // page opens straight to the pending queue instead of a blank search box.
    const initialSearch = new URLSearchParams(window.location.search).get('search') ?? '';
    const [search, setSearch] = useState(initialSearch);
    const [viewAll, setViewAll] = useState(Boolean(initialSearch));
    const [statusFilter, setStatusFilter] = useState(initialSearch ? null : 'pending');
                            ...
                            <tr>
                                <td colSpan={5} className="border-brandNavy/8 dark:border-slate-800 py-12 text-center text-brandNavy/40 dark:text-slate-500">
                                    <div className="flex flex-col items-center justify-center space-y-2">
                                        ...
                                        <span>{loading ? 'Searching…' : query ? 'No matching documents.' : statusFilter === 'pending' ? 'No pending documents.' : statusFilter ? 'No matching documents.' : 'Search or choose a status to view submitted documents.'}</span>
                                    </div>
                                </td>
                            </tr>
                        ) : (
