...
// The section add/edit modal, pulled out of SectionEditor so it can also be
// opened from the Faculty Loading rollup (editing a section from a faculty
// member's schedule, not just from its subject's own section list). The
// draft is expected to already carry subject_id and school_year — the
...
export default function SectionEditModal({ draft, setDraft, rooms, faculty, onClose, onSaved, onListsChanged }) {
    ...
    const close = () => { if (!saving) { onClose(); setError(null); setNewFaculty(null); setNewRoom(null); } };
