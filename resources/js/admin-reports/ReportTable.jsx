...
function exportCSV(rows, schoolYear, semesterLabel) {
    ...
    a.href = url;
    a.download = `AITSA_Report_AY${schoolYear}_${semesterLabel}Sem.csv`;
    a.click();
    URL.revokeObjectURL(url);
}

export default function ReportTable({ rows, schoolYear, semesterLabel }) {
        ...
        <>
            <div className="flex flex-col sm:flex-row gap-3 no-print">
                ...
                <div className="flex items-center gap-2 sm:ml-auto">
                    <button onClick={() => exportCSV(filtered, schoolYear, semesterLabel)} className="ui-btn-primary bg-brandGreen hover:bg-brandNavy text-white transition-colors">
            ...
            <div className="bg-white dark:bg-panelDark border border-brandNavy/10 dark:border-slate-800 rounded-lg shadow-sm overflow-hidden">
                <div className="px-5 py-4 border-b border-brandNavy/10 dark:border-slate-800 flex items-center justify-between">
                    ...
                    <table className="ui-table">
                        <thead>
                            <tr className="border-brandNavy/8 dark:border-slate-800 text-brandNavy/50 dark:text-slate-500">
                                ...
                                <th className="border-brandNavy/8 dark:border-slate-800 text-center">Chair</th>
                        ...
                        <tbody>
                            ...
                            {filtered.map((r, i) => (
                                <tr key={r.id}>
                                    <td className="border-brandNavy/8 dark:border-slate-800 font-mono text-xs text-brandNavy/40 dark:text-slate-500">{String(i + 1).padStart(3, '0')}</td>
                                    ...
                                    <td className="border-brandNavy/8 dark:border-slate-800 font-mono text-xs text-brandNavy/60 dark:text-slate-400">{r.studentNo}</td>
